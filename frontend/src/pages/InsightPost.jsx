  ...
  if (error) {
    return (
      <div className="pt-40 pb-40 text-center bg-[color:var(--kb-bone)]">
        <div className="font-serif-kr text-4xl">글을 찾을 수 없습니다.</div>
        <Link to="/insights" className="mt-8 inline-block text-[color:var(--kb-gold)] tick-arrow">인사이트로 돌아가기</Link>
      </div>
    );
  }
  if (!post) {
    return <div className="pt-40 pb-40 text-center bg-[color:var(--kb-bone)] text-[color:var(--kb-muted)]">불러오는 중...</div>;
  }
      ...
      <section className="bg-[color:var(--kb-bone)] py-24 md:py-32">
        <div className="max-w-[860px] mx-auto px-6 lg:px-10">
          ...
          <div className="mt-20 pt-10 border-t border-[color:var(--kb-border)] flex flex-col sm:flex-row justify-between gap-6">
            <div>
              <div className="text-[10px] tracking-[0.3em] uppercase text-[color:var(--kb-gold)]">Author</div>
              <div className="mt-2 font-serif-kr text-2xl">{post.author}</div>
            </div>
            <Link to="/insights" className="text-[color:var(--kb-ink)] tick-arrow text-sm tracking-[0.25em] uppercase self-end">모든 인사이트</Link>
          </div>
        </div>
      </section>
    </div>
