...
const CONTENT = {
  ko: {
    ...
    footer: "실제 사례는 클라이언트의 프라이버시를 존중하며 순차적으로 업데이트됩니다. 직접 사례 자료를 요청하실 수도 있습니다.",
    cta: "사례 자료 요청",
  },
  en: {
    eyebrow: "Success Stories",
    ...
    cases: [
      { category: "F&B Franchise", region: "West Coast", status: "Coming soon" },
      { category: "Retail Brand", region: "East Coast", status: "Coming soon" },
      { category: "K-Beauty", region: "Multi-city", status: "Coming soon" },
      { category: "Café Concept", region: "West Coast", status: "Coming soon" },
    ],
    challenge: "Challenge", approach: "Approach", outcome: "Outcome",
    footer: "Case studies are published incrementally to respect client privacy. You can also request materials directly.",
      ...
      <section className="bg-[color:var(--kb-bone)] py-24 md:py-32">
        <div className="max-w-[1440px] mx-auto px-6 lg:px-10">
              ...
              <FadeUp key={i} delay={i * 0.08}>
                <div className="group relative border border-[color:var(--kb-border)] bg-white overflow-hidden aspect-[4/3] p-10 flex flex-col justify-between hover:border-[color:var(--kb-gold)] transition-colors">
                  <div className="flex items-start justify-between">
                    <span className="w-10 h-px bg-[color:var(--kb-gold)] mt-4" />
                    <span className="text-[10px] tracking-[0.3em] uppercase text-[color:var(--kb-muted)] border border-[color:var(--kb-border)] px-3 py-1.5">{c.status}</span>
                  </div>
                  <div>
                    ...
                    <div className="mt-6 grid grid-cols-3 gap-6 text-xs text-[color:var(--kb-muted)]">
                      <div><div className="tracking-[0.2em] uppercase mb-1">{t.challenge}</div><div className="h-px bg-[color:var(--kb-border)] w-full" /></div>
                      <div><div className="tracking-[0.2em] uppercase mb-1">{t.approach}</div><div className="h-px bg-[color:var(--kb-border)] w-full" /></div>
                      <div><div className="tracking-[0.2em] uppercase mb-1">{t.outcome}</div><div className="h-px bg-[color:var(--kb-border)] w-full" /></div>
                    </div>
                  </div>
                </div>
              </FadeUp>
          ...
          <div className="mt-24 text-center">
            <p className="text-[color:var(--kb-muted)] max-w-xl mx-auto">{t.footer}</p>
            <div className="mt-10">
