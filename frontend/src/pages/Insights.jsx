    ...
    <div>
      ...
      <section className="bg-[color:var(--kb-bone)] py-24 md:py-32">
        <div className="max-w-[1440px] mx-auto px-6 lg:px-10">
          {loading && (
            <div className="text-center text-[color:var(--kb-muted)] py-24" data-testid="insights-loading">
              불러오는 중...
            </div>
          )}

          {!loading && posts.length === 0 && (
            <div className="text-center text-[color:var(--kb-muted)] py-24" data-testid="insights-empty">
              게시글이 준비되고 있습니다.
            </div>
          )}
          ...
          {featured && (
            <FadeUp>
              <Link
                ...
                data-testid={`${TID.insightCard}-featured`}
              ...
              >
                ...
                <div className="col-span-12 lg:col-span-5 flex flex-col justify-center">
                  ...
                  <h2 className="font-serif-kr text-3xl md:text-5xl font-light leading-tight group-hover:text-[color:var(--kb-gold)] transition-colors">
                    {featured.title}
                  </h2>
                  ...
                  <div className="mt-8 flex items-center gap-6 text-xs tracking-[0.2em] uppercase text-[color:var(--kb-muted)]">
                    <span>{featured.reading_time}분 읽기</span>
                    <span className="w-px h-3 bg-[color:var(--kb-border)]" />
                    <span className="tick-arrow text-[color:var(--kb-ink)]">읽기</span>
                  </div>
                </div>
              </Link>
            </FadeUp>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {rest.map((p, i) => (
              <FadeUp key={p.id} delay={i * 0.05}>
                <Link
                  to={`/insights/${p.slug}`}
                  data-testid={`${TID.insightCard}-${p.slug}`}
                ...
                >
                  ...
                  <div className="p-8">
                    ...
                    <h3 className="font-serif-kr text-2xl font-light leading-tight group-hover:text-[color:var(--kb-gold)] transition-colors">
                      {p.title}
                    </h3>
                    <p className="mt-4 text-sm text-[color:var(--kb-muted)] leading-relaxed line-clamp-3">
                      {p.excerpt}
                    </p>
                    <div className="mt-6 text-[11px] tracking-[0.25em] uppercase text-[color:var(--kb-ink)] tick-arrow">
                      {p.reading_time}분 읽기
                    </div>
                  </div>
                </Link>
